...
def ML_estim_func(Y, X_pca, X_nlight):
    ...
    models = [Ridge(),RandomForestRegressor(random_state=77), 
              BayesianRidge(),
              LinearRegression(),
              XGBRegressor(learning_rate=0.015, random_state=77)]

    estimates_5k_1 = np.zeros(len(models))
    estimates_10k_1 = np.zeros(len(models))
    estimates_max_1 = np.zeros(len(models))

    estimates_5k_2 = np.zeros(len(models))
    estimates_10k_2 = np.zeros(len(models))
    estimates_max_2 = np.zeros(len(models))
    ...
    dataset_ML = pd.DataFrame({'5-Fold': estimates_5k_1, 
                            ...
                            'Maximum': estimates_max_1}, columns=['5-Fold', '10-Fold','Maximum'],
                             index = ['Ridge','Random Forest', 'Bayesian Ridge', 'Linear Regression', 'XGBoost'])
